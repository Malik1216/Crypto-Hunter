                    ...
                    <div class="row mb-5">
                        <div class="col-12 p-3">
                            <a href="#"  data-toggle="modal" data-target="#faqmodal" class="btn btn-{{$text}}"><i class="fa fa-plus"></i> Add FAQ</a>
                                ...
                                <!--  -->
                                 <a href="#"  data-toggle="modal" data-target="#statmodal" class="btn btn-{{$text}}"><i class="fa fa-plus"></i> Add Statistics</a>
                                    ...
                                    <div class="modal-dialog">
                                        ...
                                        <div class="modal-body bg-{{Auth('admin')->User()->dashboard_style}}">
                                            <form action="{{route('savefaq')}}" method="POST">
                                                ...
                                                <div class="form-group">
                                                    <h5 class="text-{{$text}}">Total deposit</h5>
                                                    <input type="text" name="trading_deposit" placeholder="Enter the Question here" class="form-control bg-{{Auth('admin')->User()->dashboard_style}} text-{{$text}}" required value="{{$statistic->trading_deposit}}">
                                                </div>
                                                <div class="form-group">
                                                    <h5 class="text-{{$text}}">Trading days</h5>
                                                    <input type="text" name="trading_days" placeholder="Enter the Question here" class="form-control bg-{{Auth('admin')->User()->dashboard_style}} text-{{$text}}" required value="{{$statistic->trading_day}}">
                                                </div>
                                                <div class="form-group">
                                                    <h5 class="text-{{$text}}">Daily profit in forex</h5>
                                                    <input type="text" name="forex" placeholder="Enter the Question here" class="form-control bg-{{Auth('admin')->User()->dashboard_style}} text-{{$text}}" required value="{{$statistic->forex}}">
                                                </div>
                                                <div class="form-group">
                                                    <h5 class="text-{{$text}}">Daily profit in crypto</h5>
                                                    <input type="text" name="crypto" placeholder="Enter the Question here" class="form-control bg-{{Auth('admin')->User()->dashboard_style}} text-{{$text}}" required value="{{$statistic->crypto}}">
                                                </div>
                                                <div class="form-group">
                                                    <h5 class="text-{{$text}}">Daily profit in stocks</h5>
                                                    <input type="text" name="stocks" placeholder="Enter the Question here" class="form-control bg-{{Auth('admin')->User()->dashboard_style}} text-{{$text}}" required value="{{$statistic->stock}}">
                                                </div>
                                                <div class="form-group">
                                                    <h5 class="text-{{$text}}">Average daily profit</h5>
                                                    <input type="text" name="profit" placeholder="Enter the Question here" class="form-control bg-{{Auth('admin')->User()->dashboard_style}} text-{{$text}}" required value="{{$statistic->profit}}">
                                                </div>
                                                <div class="form-group">
                                                    <h5 class="text-{{$text}}">Weekly profit in holding</h5>
                                                    <input type="text" name="wprofit" placeholder="Enter the Question here" class="form-control bg-{{Auth('admin')->User()->dashboard_style}} text-{{$text}}" required value="{{$statistic->wprofit}}">
                                                </div>
                                                <div class="form-group">
                                                    <h5 class="text-{{$text}}">Drawdown</h5>
                                                    <input type="text" name="drawdown" placeholder="Enter the Question here" class="form-control bg-{{Auth('admin')->User()->dashboard_style}} text-{{$text}}" required value="{{$statistic->drawdown}}">
                                                </div>
                                        ...
                                        @foreach($faqs as $faq)
                                            <div class="col-md-4 p-1">
                                                <div class="card border p-1 bg-{{Auth('admin')->User()->dashboard_style}}">
                                                    <div class="card-body">
                                                        <span class="badge badge-secondary"> <small> <strong class="text-uppercase">Reference Key:</strong>  {{$faq->ref_key}}</small></span>
                                            ...
                                            @foreach($images as $faq)
                                                <div class="col-md-4 p-1">
                                                    <div class="card border p-1 bg-{{Auth('admin')->User()->dashboard_style}}">
                                                        ...
                                                        <img src="{{$settings->site_address}}/cloud/app/images/{{$faq->img_path}}" class="card-img-top w-50" alt="...">
                                                        <div class="card-body">
                                                            <h5 class="card-title text-{{$text}}"><strong>{{$faq->title}}</strong> </h5>
                                                            <p class="card-text text-{{$text}}">{{$faq->description}}</p>
                                                        </div>
