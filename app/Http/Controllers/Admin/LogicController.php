...
function RandomStringGenerator($n) 
    ...
    public function savefaq(Request $request){
      ...
      if($request['stat']){
          ...
          statistics::where('id', 1)
        ->update([
          'trading_deposit' => $request['trading_deposit'],
          'trading_days' => $request['trading_days'],
          'forex' => $request['forex'],
          'crypto' => $request['crypto'],
          'stock' => $request['stocks'],
          'profit' => $request['profit'],
          'wprofit' => $request['wprofit'],
          'drawdown' => $request['drawdown'],
        ]);
        
         return redirect()->back()
      ->with('message', 'Statistic Updated Sucessfully!');
      }
      $faq=new Faqs();
      $faq->ref_key = $String;
    ...
    //   $upload_dir="$settings->files_key/cloud/uploads";
      $upload_dir="home/images"; //fjt adds
